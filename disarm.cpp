...
//----------------------------------------
void doDisassemble(char* filename) {
	...
	while (fread(&wordIns, 4, 1, inputFile)) {
		outputstream << disInstruction(wordIns, InsCount) << "\n";
		InsCount++;
	}
	cout << outputstream.str(); // print out the result to the screen
	fclose(inputFile);
	return;
}
...
int main(int ac, char * av[]) {
	if (ac != 2) {
		cout << "\nUsage: ";
		cout << av[0] << " <input_file.bin> >> <output_file.asm>\nEg:\n" << av[0] << " abc.bin >> xyz.txt\n";
		return 1;
	}
